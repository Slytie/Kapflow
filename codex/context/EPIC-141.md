...
- `TASK-0403` is closed as of 2026-06-23: `docs/planning/capex_source_ingest/ARTIFACT_POINTER_EVENT_POLICY_CONTRACT.yaml`, `artifact_pointer_events`, `artifact_pointer_family_policies`, runtime schemas, and `onetruth.infrastructure.repositories.artifact_pointer_events` record append-only pointer history and project-scoped pointer-family policy foundation without mutating current `artifact_pointers` or emitting timeline events.
- `TASK-0405` is closed as of 2026-06-23: `docs/planning/capex_generated_artifacts/SUBMITTED_RUNTIME_GENERATED_ARTIFACT_CONTRACT.yaml`, `schemas/runtime/capex_submitted_generated_artifact.schema.json`, `schemas/runtime/capex_runtime_generated_artifact_view.schema.json`, and `onetruth.capex_platform.generated_artifact_submission` record the SubmittedGeneratedArtifact / RuntimeGeneratedArtifactView boundary while reusing the existing CAPEX generated-artifact envelope helper.
- `TASK-0402` remains open because its source dependency includes `W2-S16` / `TASK-0406`, which is still open. This does not close parser adapters, extraction/OCR runtime, queue workers, search service runtime, vector store activation, retrieval runtime, source locator unions, archive extraction runtime, upload behavior, external relay/bus wiring, broad worker migration, broad command-enforcement wiring, pointer-promotion service work, validation-run vector work, runtime-computed provenance hardening, public CAPEX activation, or reviewed evidence sufficiency.
