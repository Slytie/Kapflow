...
- update weekly/live operating-model docs so the shared popup entry surface is described without blurring workflow ownership
- record that removing the visible scheduler task does not remove canonical task/execution/input truth
...
- weekly/live operating-model docs
...
3. Freeze prerequisite truth: Stage04 inputs remain canonical gates even after the manual scheduler CTA is removed.
4. Freeze the stop line: no phone numbers in driver capabilities, no popup-only status, no browser-side ranking, no long-term weekly ownership of post-publish repair.
5. Add the epic, context packs, task briefs, and repo-memory updates in one coherent tranche.

## Verification
- `rg -n "EPIC-135|TASK-0225|shared popup|pre-publish|post-publish|driver_contact_directory|0 -> N" docs/planning docs/status codex/context codex/tasks`
- `git diff --check`

## Acceptance criteria
- EPIC-135 exists as the selected next app-facing workpage epic.
- The repo contains one long-form plan doc, one epic file, two context packs, and eight task briefs for the tranche.
- `CURRENT_FOCUS`, `DECISIONS_SINCE_LAST`, `EPICS.md`, and `TASK_INDEX.md` point fresh sessions at EPIC-135.
- Weekly/live operating-model docs now describe the shared operator surface without widening workflow ownership.
