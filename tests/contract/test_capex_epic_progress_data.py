...
def test_capex_epic_progress_data_uses_v2_estimates() -> None:
    ...
    assert data["summary"]["estimate"]["remainingTasks"] == 372
    ...
    assert epic139["displayStatus"] == "not_started"
    assert epic139["counts"]["not_started"] == 17
    assert epic139["counts"]["needs_review"] == 0
    assert epic139["estimate"]["label"] == "ETA needs completion timestamp history"


def test_epic139_redo_posture_is_non_authoritative_when_tasks_are_open() -> None:
    data = _load_data()
    ...
    assert task_statuses["TASK-0576"] == "not_started"
    assert all(status == "not_started" for status in task_statuses.values())

    assert epics["EPIC-140"]["displayStatus"] == "done"
    assert epics["EPIC-143"]["displayStatus"] == "not_started"
    assert epics["EPIC-150"]["displayStatus"] == "not_started"
    assert epics["EPIC-151"]["displayStatus"] == "not_started"
    for epic_id in ("EPIC-140", "EPIC-143", "EPIC-150", "EPIC-151"):
...
def test_historical_done_tasks_report_missing_completion_timestamps() -> None:
    ...
    assert {task["id"] for task in historical_missing} >= {"TASK-0233", "TASK-0234"}
