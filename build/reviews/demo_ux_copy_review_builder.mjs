...
const GLOSSARY_ROWS = [
  ...
  ["workflow run", "run", "Use when operators need to know they are looking at one specific schedule or reporting run.", "Avoid 'workflow-run-backed' phrasing and raw ids in primary UI copy."],
  ...
  ["Stage03 or Stage04", "draft step or review step", "Use only if support text truly needs to explain where the user is in the draft or review flow.", "Prefer business steps over numbered stages in primary UI copy."],
  ["UPD", "UPD (>600-minute review)", "Use where operators may know UPD already but still benefit from a plain-English reminder.", "Avoid leaving the acronym unexplained in first-view copy."],
  ["route demand", "route demand or daily planned routes", "Keep route demand where planners already use that term; use daily planned routes when a plainer explanation helps.", "Do not replace the domain term everywhere if the audience already works with route demand daily."],
...
const EXACT_COPY_OVERRIDES = new Map([
  ...
  ["Workflow", { action: "keep", proposed: "Workflow", reason: "Workflow is already a familiar domain term in this demo context.", severity: "low", theme: "Operator-friendly as is" }],
  ["Workflow run", { action: "rewrite", proposed: "Run", reason: "Use the shorter run label instead of workflow-run terminology.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Viewer session", { action: "rewrite", proposed: "Current access", reason: "If this label stays visible, it should describe the access context rather than an internal session object.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Choose a workflow run to open drill-down.", { action: "rewrite", proposed: "Choose a linked run to open details.", reason: "Keep the linked-run context and remove drill-down language.", severity: "medium", theme: "Internal platform jargon" }],
  ["Choose a workflow run", { action: "rewrite", proposed: "Choose a linked run", reason: "This selector is choosing among linked runs, not raw workflow-run objects.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Loading workflow drill-down", { action: "rewrite", proposed: "Loading linked run details", reason: "The panel is about linked-run details; drill-down is interface jargon.", severity: "medium", theme: "Internal platform jargon" }],
  ["No drill-down runs available.", { action: "rewrite", proposed: "No linked runs are available.", reason: "Say exactly what is missing instead of using drill-down wording.", severity: "medium", theme: "Internal platform jargon" }],
  ["Drill-down mode", { action: "rewrite", proposed: "View mode", reason: "This support label should avoid drill-down jargon.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Latest task:", { action: "rewrite", proposed: "Current task", reason: "Task is the right workflow term here; current is clearer than latest in the detail context.", severity: "low", theme: "Operator-friendly as is" }],
  ...
  ["This workflow-run-backed schedule projection is built from canonical weekly-planning input artifacts for the selected run.", { action: "rewrite", proposed: "This page uses the latest weekly planning inputs for the selected run.", reason: "Keep the focus on weekly planning inputs instead of platform authority terms.", severity: "high", theme: "Internal platform jargon" }],
  ...
  ["Open schedule draft", { action: "rewrite", proposed: "Open latest schedule draft", reason: "Make it clear that the action opens the current editable draft, not an arbitrary draft.", severity: "low", theme: "Technical versioning detail" }],
  ["This page is a weekly-planning review surface. Any selected-day controls below are local what-if inputs for the prototype and do not replace live_dispatch.v1 day-of truth.", { action: "rewrite", proposed: "This page is for weekly planning review. Any selected-day controls below are what-if inputs only and do not replace day-of dispatch decisions.", reason: "Keep the weekly-planning boundary clear without exposing workflow ids or truth-model language.", severity: "high", theme: "Stage/workflow codes" }],
  ["Selected-day controls are local what-if inputs only and do not claim ownership of live dispatch truth.", { action: "rewrite", proposed: "Selected-day controls are what-if inputs only and do not change day-of dispatch.", reason: "Shorten the warning and keep the weekly-planning versus day-of-dispatch boundary clear.", severity: "high", theme: "Stage/workflow codes" }],
  ...
  ["Fetching the immutable Stage04 draft weekly schedule artifact projection.", { action: "rewrite", proposed: "Loading the saved weekly planning draft.", reason: "Use weekly-planning draft language instead of stage-code and artifact terminology in the loading state.", severity: "high", theme: "Stage/workflow codes" }],
  ...
  ["Stage04 draft boundary", { action: "rewrite", proposed: "Planning draft boundary", reason: "Use the business step instead of the numbered stage code.", severity: "medium", theme: "Stage/workflow codes" }],
  ["This page edits the immutable Stage04 draft weekly schedule workbook only. It does not claim published weekly truth, does not replace manager-review evidence, and does not reach into live_dispatch.v1 day-of control.", { action: "rewrite", proposed: "This page only edits the saved weekly planning draft. It does not publish the weekly schedule or control day-of dispatch.", reason: "Keep the schedule boundary clear without exposing stage codes or workflow ids.", severity: "high", theme: "Stage/workflow codes" }],
  ["Published weekly truth still begins only at Stage06 pointer promotion, and live dispatch remains downstream.", { action: "rewrite", proposed: "The weekly schedule is not published until final approval, and day-of dispatch still happens later.", reason: "Keep the publish boundary clear without exposing stage numbers or pointer-promotion terms.", severity: "high", theme: "Stage/workflow codes" }],
  ["This artifact-backed schedule workpage edits only the Stage04 draft weekly schedule workbook for the selected run.", { action: "rewrite", proposed: "This page only edits the saved weekly planning draft for the selected run.", reason: "Remove artifact, workpage, and stage-code wording from the schedule-draft explanation.", severity: "high", theme: "Stage/workflow codes" }],
  ...
  ["Open route demand", { action: "keep", proposed: "Open route demand", reason: "Route demand is already a familiar operator term in this workflow.", severity: "low", theme: "Operator-friendly as is" }],
  ...
  ["Weekly route demand review", { action: "keep", proposed: "Weekly route demand review", reason: "The title already uses the right domain term for this planning surface.", severity: "low", theme: "Operator-friendly as is" }],
  ["This workflow-run-backed route demand page is built from the latest canonical Stage04 route-demand artifact for the selected run.", { action: "rewrite", proposed: "This page shows the latest saved route demand for the selected run.", reason: "Keep route demand as the domain term while removing platform and stage jargon.", severity: "high", theme: "Internal platform jargon" }],
  ["Route-demand edits stay on a separate truth surface from schedule reassignment and recalculation.", { action: "rewrite", proposed: "Changes here only affect route demand. They do not reassign drivers or update the schedule.", reason: "Keep the route-demand boundary while removing truth-surface terminology.", severity: "high", theme: "Internal platform jargon" }],
  ["This page edits route-demand truth only. Schedule reassignment, preview, and save stay on schedule-v0, and route-demand saves never auto-refresh schedule artifacts.", { action: "rewrite", proposed: "This page only updates route demand. Schedule edits and previews happen on the schedule page, and saving here does not update the schedule automatically.", reason: "Keep route demand as the main term and remove route ids, artifact, and auto-refresh jargon.", severity: "high", theme: "Internal platform jargon" }],
  ["This artifact-backed route demand page edits daily route-demand truth only. It does not mutate schedule assignments or auto-refresh schedule drafts.", { action: "rewrite", proposed: "This page only updates daily route demand. It does not change driver assignments or update schedule drafts automatically.", reason: "Keep the daily route-demand boundary without artifact or mutation terminology.", severity: "high", theme: "Internal platform jargon" }],
  ["Route Demand Landing", { action: "rewrite", proposed: "Weekly route demand", reason: "Landing is a navigation term; keep route demand as the page concept.", severity: "medium", theme: "Operator-friendly as is" }],
  ["A read-only weekly landing page for backend-owned route-demand truth. Open the latest immutable artifact when you need to edit final daily counts.", { action: "rewrite", proposed: "This read-only page shows weekly route demand. Open the latest saved version when you need to edit daily route totals.", reason: "Keep route demand as the primary term and use daily route totals only as supporting explanation.", severity: "high", theme: "Internal platform jargon" }],
  ["Workflow-run-backed route-demand projection served from the latest canonical Stage04 route-demand artifact for this weekly run.", { action: "rewrite", proposed: "This page shows the latest saved route demand for this planning run.", reason: "Keep route demand as the primary term while removing workflow, stage, and artifact jargon.", severity: "high", theme: "Internal platform jargon" }],
  ["This landing page stays read-only. Open the latest immutable route-demand artifact to adjust final daily counts and create a new successor version.", { action: "rewrite", proposed: "This page is read-only. Open the latest saved route demand version to adjust daily route totals and save a new version.", reason: "Keep route demand as the primary term and simplify the save/version explanation.", severity: "high", theme: "Technical versioning detail" }],
  ["A bounded route-demand editor over immutable weekly route-demand workbooks. Saving creates the next immutable route-demand version and never mutates schedule artifacts.", { action: "rewrite", proposed: "Edit weekly route demand here. Saving creates a new route demand version and does not change the schedule.", reason: "Keep the page anchored to route demand while making the schedule boundary explicit.", severity: "high", theme: "Internal platform jargon" }],
  ["Artifact-backed route-demand projection served from an immutable Stage04 route-demand workbook version.", { action: "rewrite", proposed: "This page shows a saved route demand version for weekly planning.", reason: "Keep route demand as the domain term while removing stage and artifact wording.", severity: "high", theme: "Internal platform jargon" }],
  ["Plus/minus controls adjust backend-owned daily route counts. Save creates a new route-demand artifact version and leaves schedule artifacts untouched.", { action: "rewrite", proposed: "Use plus and minus to change daily route totals. Saving creates a new route demand version and does not change the schedule.", reason: "Keep route demand as the saved object and use daily route totals only as supporting explanation.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["This route-demand version is historical. Open the latest version in the chain before saving additional changes.", { action: "rewrite", proposed: "This route demand version is older. Open the latest version before saving more changes.", reason: "Keep route demand as the domain term and simplify the version-history language.", severity: "medium", theme: "Technical versioning detail" }],
  ["This future week already has weekly schedule draft truth. Continue from the schedule workpage instead of editing route demand here.", { action: "rewrite", proposed: "This future week already has a schedule draft. Continue from the schedule page instead of editing route demand here.", reason: "Keep route demand as the domain term while removing workpage and truth language.", severity: "high", theme: "Internal platform jargon" }],
  ["This route-demand version is historical and can no longer be edited.", { action: "rewrite", proposed: "This route demand version is older and can no longer be edited.", reason: "Keep route demand as the domain term and use plainer version language.", severity: "medium", theme: "Technical versioning detail" }],
  ...
  ["A Stage04 refresh work item is already open for the stale schedule draft. Route-demand save did not mutate any schedule artifact.", { action: "rewrite", proposed: "A schedule refresh is already open for the out-of-date draft. Saving route demand here did not update the schedule.", reason: "Keep the route-demand boundary while removing stage-code and artifact terminology.", severity: "high", theme: "Stage/workflow codes" }],
  ["The backend reported the latest schedule draft posture for this route-demand surface.", { action: "rewrite", proposed: "This shows the latest schedule-draft status for this route demand page.", reason: "Use page/status language instead of backend posture and surface wording.", severity: "medium", theme: "Internal platform jargon" }],
  ["The history rail stays within backend-authored immutable route-demand workbook lineage for this weekly run.", { action: "rewrite", proposed: "This history stays within saved route demand versions for this run.", reason: "Keep route demand as the domain term while removing lineage and backend jargon.", severity: "high", theme: "Internal platform jargon" }],
  ["Route-demand version", { action: "rewrite", proposed: "Route demand version", reason: "Keep the route demand term instead of substituting a new concept.", severity: "low", theme: "Operator-friendly as is" }],
  ["Route demand artifact context", { action: "rewrite", proposed: "Route demand version details", reason: "Version details is plainer than artifact context and still fits the history model.", severity: "medium", theme: "Technical versioning detail" }],
  ["Loading route demand artifact", { action: "rewrite", proposed: "Loading route demand version", reason: "Version is plainer than artifact in the loading state.", severity: "medium", theme: "Technical versioning detail" }],
  ["Route demand artifact failed to load", { action: "rewrite", proposed: "Route demand version failed to load", reason: "Version is plainer than artifact in the error state.", severity: "medium", theme: "Technical versioning detail" }],
  ["Route demand artifact route is incomplete", { action: "rewrite", proposed: "Route demand version route is incomplete", reason: "If this support message remains visible, version is plainer than artifact.", severity: "medium", theme: "Technical versioning detail" }],
  ["Resolving the latest route-demand artifact for this weekly run.", { action: "rewrite", proposed: "Loading the latest saved route demand version for this run.", reason: "Keep route demand as the domain term and remove artifact jargon from the quick-edit state.", severity: "medium", theme: "Technical versioning detail" }],
  ["No editable route-demand artifact is available for this weekly run yet.", { action: "rewrite", proposed: "No editable saved route demand version is available for this run yet.", reason: "Keep the save/version model without forcing artifact terminology on the user.", severity: "medium", theme: "Technical versioning detail" }],
  ...
  ["Weekly driver preferences", { action: "keep", proposed: "Weekly driver preferences", reason: "This is already the right operator-facing term for the page.", severity: "low", theme: "Operator-friendly as is" }],
  ["Create preferences snapshot", { action: "rewrite", proposed: "Save weekly preferences", reason: "Snapshot is technical save-state language.", severity: "medium", theme: "Technical versioning detail" }],
  ...
  ["Fetching the immutable driver-preferences snapshot projection.", { action: "rewrite", proposed: "Loading the saved weekly preferences view.", reason: "Use weekly preferences language instead of immutable snapshot projection wording.", severity: "high", theme: "Internal platform jargon" }],
  ...
  ["Open latest draft", { action: "rewrite", proposed: "Open latest end-of-day draft", reason: "Name the draft directly so the action is clearer in the reporting context.", severity: "low", theme: "Operator-friendly as is" }],
  ["Loading end-of-day workpage", { action: "rewrite", proposed: "Loading end-of-day report", reason: "Report is the clearer product term in this reporting flow.", severity: "medium", theme: "Internal platform jargon" }],
  ["Fetching the workflow-run-backed dispatch-reporting landing workpage.", { action: "rewrite", proposed: "Loading end-of-day reporting for this run.", reason: "Use the reporting surface name directly and remove workflow-run wording.", severity: "high", theme: "Internal platform jargon" }],
  ["End-of-day workpage failed to load", { action: "rewrite", proposed: "End-of-day report failed to load", reason: "Report is the clearer product term in this reporting flow.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Open dispatch-reporting workpages from a canonical workflow-run route.", { action: "rewrite", proposed: "Open end-of-day pages from a reporting run page.", reason: "Keep the support message tied to the reporting flow instead of route internals.", severity: "medium", theme: "Internal platform jargon" }],
  ["Open EOD drafts from a canonical workflow-run route.", { action: "rewrite", proposed: "Open end-of-day drafts from a reporting run page.", reason: "Keep the support message tied to the reporting flow instead of route internals.", severity: "medium", theme: "Internal platform jargon" }],
  ...
  ["Resolving the latest Stage04 review task for this reporting run.", { action: "rewrite", proposed: "Loading the current review step for this reporting run.", reason: "Use the business step instead of the numbered stage code in the closeout flow.", severity: "medium", theme: "Stage/workflow codes" }],
  ...
  ["Both the workflow run id and artifact version id are required.", { action: "rewrite", proposed: "Both the run id and version id are required.", reason: "Remove workflow-run and artifact-version phrasing from the support message.", severity: "high", theme: "Raw system metadata" }],
...
function replacementNotes(routeGroup) {
  ...
  if (routeGroup.startsWith("Route Demand")) {
    return "Keep route-demand editing separate from schedule editing and auto-refresh behavior.";
  }
