...
## TASK-0372 / TASK-0373 closeout evidence
- Extended `docs/planning/capex_source_ingest/BATCH_ARTIFACT_LINK_PROVENANCE_HYDRATION_CONTRACT.yaml` with the `RF-004` / `RF-005` closeout surface for workflow-run and subject page adapters.
- Extended `onetruth.infrastructure.repositories.artifact_relation_hydration` with bounded workflow-run and subject artifact page helpers, deterministic SQL-level `limit`/`offset`, optional `artifact_kind`, scoped hydration, and optional internal human-task/flag summary batching.
- Routed existing artifact list API branches through paginated read commands while preserving route shapes and response envelopes.
- Evidence: artifact relation hydration unit tests, runtime/API artifact list tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `RF-004` and `RF-005` close query/projection refactor evidence only; bulk ingest adapter seams, public routes, frontend routes, migrations, event-registry changes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0374 / TASK-0391 / TASK-0392 / TASK-0393 closeout evidence
- Extended `docs/planning/capex_source_ingest/BULK_STAGED_CORPUS_INGEST_ARCHITECTURE.yaml` with the `RF-006` closeout surface for the internal planning-only bulk ingest adapter seam.
- Added `onetruth.capex_platform.bulk_ingest_adapter_seam` to validate tenant/domain/project scope, adapter request ID, idempotency key, actor, timestamp, supported descriptor modes, duplicate descriptor IDs, raw-material bans, deterministic descriptor fingerprints, and no runtime/official truth effects while reusing `plan_staged_corpus_ingest(...)`.
- Added `docs/planning/capex_source_ingest/CONTENT_IDENTITY_SOURCE_OCCURRENCE_RUNTIME_SCHEMA_RECONCILIATION.yaml` to reconcile `ARCH-W2-S01` to existing `TASK-0564` physical schema evidence instead of adding a duplicate migration.
- Added `docs/planning/capex_source_ingest/SOURCE_OCCURRENCE_RELATION_CONTRACT.yaml`, `schemas/runtime/capex_source_occurrence_relation.schema.json`, and `onetruth.infrastructure.repositories.capex_source_occurrence_relations` for same-project duplicate/archive/derivative/redaction relation rows over existing source occurrences.
- Added `docs/planning/capex_source_ingest/INGEST_JOB_STATE_MODEL_CONTRACT.yaml`, ingest batch/job/attempt/log runtime schemas, and `onetruth.infrastructure.repositories.capex_ingest_jobs` for scoped ingest state rows with monotonic attempts and terminal-state guards.
- Evidence: bulk ingest adapter unit tests, source occurrence relation unit tests, ingest job state unit tests, source occurrence and ingest schema parity tests, migration/bootstrap parity tests, runtime DB model contract tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `RF-006` and `ARCH-W2-S01..S03` close planning/helper/schema/state evidence only; locator unions, archive extraction runtime, extractor/OCR/search runtime, queue workers, public routes, frontend routes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0394 / TASK-0395 closeout evidence
- Added `docs/planning/capex_source_ingest/ARCHIVE_LINEAGE_METADATA_CONTRACT.yaml` and `onetruth.capex_platform.archive_lineage_metadata` for deterministic archive lineage and nested archive member metadata outputs over existing source occurrence and relation rows.
- Added `docs/planning/capex_source_ingest/MANIFEST_GENERATION_ATTESTATION_CONTRACT.yaml` and `onetruth.capex_platform.manifest_generation_attestation` for deterministic generated corpus register manifest and manifest-generation attestation outputs from physical content identity, source occurrence, and relation rows only.
- Evidence: archive lineage metadata unit tests, manifest-generation attestation unit tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `ARCH-W2-S04` and `ARCH-W2-S05` close metadata/attestation evidence only; duplicate migrations, archive extraction runtime, locator unions, extractor/OCR/search runtime, queue workers, public routes, frontend routes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0396 / TASK-0397 closeout evidence
- Added `docs/planning/capex_source_ingest/COMMAND_RECEIPT_CANONICAL_INPUT_CONTRACT.yaml` and hardened the existing command receipt substrate so request fingerprints use canonical JSON bytes, `sha256:` digests, and the stored `onetruth.command_receipt_input.canonical_json.sha256.v1` profile.
- Added `docs/planning/capex_source_ingest/EFFECT_LEDGER_GUARDED_MUTATION_CONTRACT.yaml`, `effect_ledger_entries`, `schemas/runtime/effect_ledger_entry.schema.json`, `onetruth.infrastructure.repositories.effect_ledger`, and `onetruth.application.handlers._shared.effect_ledger` for deterministic command-scoped effect reservation, replay, conflict, and rollback behavior.
- Evidence: command receipt unit tests, EffectLedger unit tests, runtime DB schema/model/bootstrap parity tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `ARCH-W2-S06` and `ARCH-W2-S07` close internal runtime foundation evidence only; broad command-handler rewiring, tool-execution splitting, runtime outbox dispatch, parser/OCR/search runtime, queue workers, public routes, frontend routes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0398 / TASK-0399 closeout evidence
- Added `docs/planning/capex_source_ingest/TOOL_EXECUTION_ATTEMPT_RUNTIME_CONTRACT.yaml`, `tool_execution_attempts`, `schemas/runtime/tool_execution_attempt.schema.json`, and `onetruth.infrastructure.repositories.tool_execution_attempts` for the internal logical tool execution / leased attempt split.
- Extended `onetruth.application.handlers.execution_runtime` with `start_tool_execution_attempt_command(...)` and leased completion guards. Legacy completions still work when no active attempt exists; active attempts require matching `tool_execution_attempt_id` and `lease_token`, and stale/missing leases fail closed without logical completion or completion-event emission.
- Added `docs/planning/capex_source_ingest/PROJECT_CONCURRENCY_RUNTIME_SLOT_CONTRACT.yaml`, `capex_project_concurrency_policies`, `capex_project_runtime_slots`, runtime schemas, and `onetruth.infrastructure.repositories.capex_project_runtime_slots` for project-scoped `ingest` and `pointer` lock primitives.
- Evidence: ToolExecutionAttempt unit tests, project runtime slot unit tests, execution-runtime compatibility tests, runtime DB schema/model/bootstrap parity tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `ARCH-W2-S08` and `ARCH-W2-S09` close internal runtime foundation evidence only; broad worker migration, runtime outbox dispatch, broad command-enforcement wiring, parser/OCR/search runtime, queue workers, public routes, frontend routes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0400 / TASK-0401 closeout evidence
- Added `docs/planning/capex_source_ingest/RUNTIME_OUTBOX_AFTER_COMMIT_CONTRACT.yaml` and `onetruth.infrastructure.repositories.runtime_outbox` for an internal after-commit relay scaffold over canonical `timeline_events` plus `consumer_cursors`, without adding a second outbox table.
- RuntimeOutbox reads committed events after a tenant/domain-scoped consumer cursor, dispatches in deterministic `sequence_no` order, supports bounded batches and optional event-type filtering, advances past skipped committed events, and leaves the cursor before failed events for retry.
- Added `docs/planning/capex_source_ingest/ARTIFACT_VERSION_IDENTITY_CONTRACT.yaml`, Alembic revision `20260624_0021`, SQLite bootstrap/model parity, and `onetruth.infrastructure.repositories.artifact_versions` identity helpers for `artifact_versions.artifact_identity_profile` and `artifact_versions.artifact_identity_digest`.
- ArtifactVersion identity digests are immutable `sha256:` metadata over canonical scope/content identity fields only. Storage URI, pointer state, mutable status, and officialness are excluded, and no officialness-like field is added to `artifact_versions`.
- Evidence: RuntimeOutbox unit tests, ArtifactVersion identity unit tests, generated-artifact helper and artifact-pointer identity regressions, runtime DB schema/model/bootstrap parity tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `ARCH-W2-S10` and `ARCH-W2-S11` close internal relay scaffold and artifact identity metadata evidence only; external relay/bus activation, broad worker migration, broad command-enforcement wiring, pointer-promotion service work, parser/OCR/search runtime, queue workers, public routes, frontend routes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.

## TASK-0403 / TASK-0405 closeout evidence
- Added `docs/planning/capex_source_ingest/ARTIFACT_POINTER_EVENT_POLICY_CONTRACT.yaml`, `artifact_pointer_events`, `artifact_pointer_family_policies`, runtime schemas, Alembic revision `20260624_0022`, SQLite bootstrap/model parity, and `onetruth.infrastructure.repositories.artifact_pointer_events` for append-only pointer history and minimal pointer-family policy foundation.
- Added `docs/planning/capex_generated_artifacts/SUBMITTED_RUNTIME_GENERATED_ARTIFACT_CONTRACT.yaml`, `schemas/runtime/capex_submitted_generated_artifact.schema.json`, `schemas/runtime/capex_runtime_generated_artifact_view.schema.json`, and `onetruth.capex_platform.generated_artifact_submission` for the SubmittedGeneratedArtifact / RuntimeGeneratedArtifactView schema boundary.
- Evidence: ArtifactPointerEvent/policy unit tests, generated-artifact submission/view unit tests, existing pointer identity and generated-artifact envelope/validator regressions, runtime DB schema/model/bootstrap parity tests, CAPEX ingest/generated-artifact contract tests, and progress-data contract tests cover the closeout.
- Closeout posture: `ARCH-W2-S13` and `ARCH-W2-S15` close dependency-ready internal schema/helper evidence only. `TASK-0402` remains open until `TASK-0406` closes; pointer-promotion service behavior, current pointer mutation, broad generated-artifact migration, public routes, frontend routes, event-registry changes, raw corpus import, reviewed baseline truth, official pointer creation, and CAPEX runtime/product activation remain later gated work.
...
- Closeout posture: `ART-002` closes CEO-safe generated artifact shape evidence only; raw AI output, raw corpus fields, false precision when not forecastable, CEO cockpit activation, public routes, frontend routes, runtime risk engine, official pointer creation, closure snapshots, and CAPEX runtime/product activation remain later gated work. Follow-on planning evidence for RiskSignal and W8 freshness is now closed under `TASK-0539` and `TASK-0540`.
...
- Closeout posture: `WFLOW-008` closes planning/internal risk and CEO transparency output-shape evidence only; CEO cockpit/workpage, public routes, frontend routes, authored workflow activation, migrations, event-registry changes, official pointer creation, closure snapshots, external-system activation, and CAPEX runtime/product activation remain later gated work. Planning RiskSignal and W8 CEO freshness evidence is now closed under `TASK-0539` and `TASK-0540`.

## TASK-0539 closeout evidence
- Added `docs/planning/capex_transparency/RISK_SIGNAL_CONTRACT.yaml` for the `ARCH-W8-S03` RiskSignal planning contract.
- Added `onetruth.capex_platform.risk_signal` to produce deterministic `capex.risk_signal_register.v1` rows from `capex.risk_ceo_transparency.workflow_outputs.v1`, including predicate ids, risk refs, severity/status enums, owner roles, SourceRefs, policy version, input digests, row digests, and register digest.
- Evidence: RiskSignal unit tests and CAPEX semantic/transparency contract tests passed on 2026-06-23.
- Closeout posture: `ARCH-W8-S03` closes planning-only RiskSignal register evidence; runtime risk engine activation, CEO cockpit runtime, public routes, frontend routes, migrations, event-registry changes, official pointer creation, closure snapshots, external-system activation, raw corpus import, and CAPEX runtime/product activation remain later gated work.

## TASK-0540 closeout evidence
- Added `docs/planning/capex_transparency/CEO_TRANSPARENCY_SNAPSHOT_W8_FRESHNESS_CONTRACT.yaml` and `schemas/runtime/capex_ceo_transparency_snapshot_freshness.schema.json` for the `ARCH-W8-S04` W8 CEO transparency freshness companion.
- Extended `onetruth.capex_platform.ceo_transparency_snapshot` with `build_ceo_transparency_snapshot_freshness_outputs(...)` over the existing `capex.ceo_transparency_snapshot.v1` payload, Risk/CEO workflow outputs, and optional RiskSignal outputs.
- Evidence: CEO transparency snapshot freshness unit tests, CAPEX generated-artifact contract tests, and CAPEX semantic/transparency contract tests passed on 2026-06-23.
- Closeout posture: `ARCH-W8-S04` does not replace the `TASK-0277` CEO snapshot artifact and creates no CEO cockpit/runtime view, runtime risk engine, public routes, frontend routes, migrations, event-registry changes, official pointer creation, closure snapshots, external-system activation, raw corpus import, or CAPEX runtime/product activation.
...
| Source occurrence / SourceRef | `TASK-0268`, `TASK-0391`, `TASK-0392`, `TASK-0394`, `TASK-0407`, `TASK-0428`, `TASK-0564` | `TASK-0391` is reconciled to existing `TASK-0564` physical `capex_content_identities` / `capex_source_occurrences` schema evidence and no duplicate migration is needed; `TASK-0268` adds deterministic source occurrence register creation from sanitized contexts; `TASK-0392` adds internal same-project source occurrence relation state; `TASK-0394` records archive lineage metadata over existing archive relation rows only; locator unions, extraction, and evidence binding remain future work |
...
| Artifact pagination and relation hydration RF closeout | `TASK-0372`, `TASK-0373` | `docs/planning/capex_source_ingest/BATCH_ARTIFACT_LINK_PROVENANCE_HYDRATION_CONTRACT.yaml`, `src/onetruth/infrastructure/repositories/artifact_relation_hydration.py`, and artifact list route/read-command tests close bounded workflow-run/subject page and optional task/flag summary hydration evidence only; bulk ingest seam, public route activation, frontend routes, migrations, raw corpus import, official pointers, reviewed baseline truth, and activation remain blocked |
| Bulk ingest adapter seam | `TASK-0374` | `src/onetruth/capex_platform/bulk_ingest_adapter_seam.py` and the staged ingest architecture `RF-006` closeout surface add an internal adapter boundary over sanitized staged descriptors only; JSON/base64 upload, local source-path ingest, artifact ingress commands, source occurrence creation, public routes, frontend routes, raw corpus import, and activation remain blocked |
| Ingest job state model | `TASK-0393` | `docs/planning/capex_source_ingest/INGEST_JOB_STATE_MODEL_CONTRACT.yaml`, runtime schemas, migration/bootstrap/model state, and `src/onetruth/infrastructure/repositories/capex_ingest_jobs.py` close internal batch/job/attempt/log state only; workers, queues, parser/OCR/search runtime, artifact creation, source occurrence creation, public routes, frontend routes, raw corpus import, and activation remain blocked |
| Manifest generation attestation | `TASK-0395` | `docs/planning/capex_source_ingest/MANIFEST_GENERATION_ATTESTATION_CONTRACT.yaml` and `src/onetruth/capex_platform/manifest_generation_attestation.py` close generated corpus register attestation from physical rows only; generated registers are not source authority, reviewed-baseline truth, evidence sufficiency, official pointers, or activation |
| Command receipt and EffectLedger runtime foundation | `TASK-0396`, `TASK-0397` | `docs/planning/capex_source_ingest/COMMAND_RECEIPT_CANONICAL_INPUT_CONTRACT.yaml`, `docs/planning/capex_source_ingest/EFFECT_LEDGER_GUARDED_MUTATION_CONTRACT.yaml`, receipt hash/profile storage, `effect_ledger_entries`, runtime schemas, and guarded mutation helpers close internal idempotency/effect foundation only; broad command rewiring, public routes, official pointers, reviewed-baseline truth, and activation remain blocked |
| Tool attempts and project runtime slots | `TASK-0398`, `TASK-0399` | `docs/planning/capex_source_ingest/TOOL_EXECUTION_ATTEMPT_RUNTIME_CONTRACT.yaml`, `docs/planning/capex_source_ingest/PROJECT_CONCURRENCY_RUNTIME_SLOT_CONTRACT.yaml`, `tool_execution_attempts`, `capex_project_concurrency_policies`, `capex_project_runtime_slots`, runtime schemas, and internal repositories close leased attempt and ingest/pointer slot foundation only; broad worker migration, runtime outbox dispatch, broad command enforcement, public routes, official pointers, reviewed-baseline truth, and activation remain blocked |
| RuntimeOutbox and ArtifactVersion identity | `TASK-0400`, `TASK-0401` | `docs/planning/capex_source_ingest/RUNTIME_OUTBOX_AFTER_COMMIT_CONTRACT.yaml`, `docs/planning/capex_source_ingest/ARTIFACT_VERSION_IDENTITY_CONTRACT.yaml`, `src/onetruth/infrastructure/repositories/runtime_outbox.py`, and `artifact_versions.artifact_identity_profile` / `artifact_identity_digest` close internal relay scaffold and immutable artifact identity metadata only; external relay/bus activation, broad worker migration, pointer-promotion service work, public routes, official pointers, reviewed-baseline truth, and activation remain blocked |
| Pointer history and generated artifact schema boundary | `TASK-0403`, `TASK-0405` | `docs/planning/capex_source_ingest/ARTIFACT_POINTER_EVENT_POLICY_CONTRACT.yaml`, `docs/planning/capex_generated_artifacts/SUBMITTED_RUNTIME_GENERATED_ARTIFACT_CONTRACT.yaml`, `artifact_pointer_events`, `artifact_pointer_family_policies`, and `src/onetruth/capex_platform/generated_artifact_submission.py` close append-only pointer-history/policy and submitted/runtime generated-artifact boundary evidence only; pointer promotion service, current pointer mutation, broad generated-artifact migration, public routes, official pointers, reviewed-baseline truth, and activation remain blocked |
...
| CEO Transparency Snapshot artifact | `TASK-0277` | `docs/planning/capex_generated_artifacts/CEO_TRANSPARENCY_SNAPSHOT_CONTRACT.yaml`, `schemas/runtime/capex_ceo_transparency_snapshot.schema.json`, and `src/onetruth/capex_platform/ceo_transparency_snapshot.py` close CEO-safe planning artifact shape evidence only; CEO cockpit UI, public routes, official pointers, closure snapshots, and activation remain blocked |
...
| RiskSignal planning register | `TASK-0539` | `docs/planning/capex_transparency/RISK_SIGNAL_CONTRACT.yaml` and `src/onetruth/capex_platform/risk_signal.py` close deterministic RiskSignal register evidence only; runtime risk engine, CEO cockpit runtime, public routes, official pointers, closure snapshots, external systems, and activation remain blocked |
| CEO Transparency Snapshot W8 freshness companion | `TASK-0540` | `docs/planning/capex_transparency/CEO_TRANSPARENCY_SNAPSHOT_W8_FRESHNESS_CONTRACT.yaml`, `schemas/runtime/capex_ceo_transparency_snapshot_freshness.schema.json`, and `src/onetruth/capex_platform/ceo_transparency_snapshot.py` close W8 freshness companion evidence only; the `TASK-0277` CEO snapshot artifact is not replaced and runtime/product activation remains blocked |
