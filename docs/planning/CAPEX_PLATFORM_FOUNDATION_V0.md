...
- Source occurrence and SourceRef runtime foundation is present through `TASK-0564`; broader corpus ingest, source occurrence relation/locator work, extraction, and evidence binding remain blocked until later EPIC-141 tasks close.
- Workflow handoff manifest and workpage projection/stale-command foundations are present through `TASK-0566` and `TASK-0567`; authored CAPEX workflow packs, public CAPEX workpage APIs, frontend CAPEX workpage routes, projection hydration families, and runtime activation remain blocked until later EPIC-143/144 tasks close.
- CAPEX semantic test and interface-burden foundations are present through `TASK-0568` and `TASK-0569`; hosted branch-protection enforcement, richer review-tier automation, public interface queues, workflow/workpage routing, raw-corpus governance dependencies, and runtime activation remain blocked until later tasks close.
