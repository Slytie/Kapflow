...
- The bounded route-demand coverage closeout on `2026-05-10` also required a backend-owned frontend snapshot refresh before the route-demand and schedule artifact fixture checks returned green again.
...
## Route-Demand Coverage Addendum (2026-05-10)
Additional bounded EPIC-135 subset verification commands executed:
- `git diff --check` - passed
- `PYTHONPATH=src python3.11 -m pytest -q tests/unit/test_schedule_control_route_demand_coverage_recommendations.py tests/unit/test_schedule_draft_workbook.py tests/unit/test_api_route_registry.py tests/runtime/api/test_workpages_route_demand_contract.py tests/runtime/api/test_workpages_artifact_schedule_contract.py` - passed
- `npm --prefix frontend run typecheck` - passed
- `npm --prefix frontend run test:run -- src/pages/logisticsRouteDemandWorkpagePage.test.tsx src/pages/logisticsScheduleArtifactWorkpagePage.test.tsx` - passed
- `make frontend-workpages-smoke` - passed
- `make PYTHON=python3.11 workpage-mutation-smoke` - passed
- `PYTHONPATH=src python3.11 scripts/export_frontend_snapshots.py` - passed (regenerated affected backend-owned workpage fixtures)
- `PYTHONPATH=src python3.11 scripts/export_frontend_snapshots.py --check` - passed
- `PYTHONPATH=src python3.11 -m pytest -q tests/runtime/contracts/test_frontend_snapshot_fixtures.py` - passed
- `python3.11 scripts/validate_repo.py --schemas-only` - passed
...
| Pre-publish route-demand coverage handoff over `schedule-v0` | `POST /api/v1/workpages/workflow-runs/{workflow_run_id}/route-demand-v0/artifacts/{artifact_version_id}/save-and-run`<br>`POST /api/v1/workpages/workflow-runs/{workflow_run_id}/schedule-v0/artifacts/{artifact_version_id}/route-demand-coverage-candidates`<br>`POST /api/v1/workpages/workflow-runs/{workflow_run_id}/schedule-v0/artifacts/{artifact_version_id}/route-demand-coverage` | `tests/unit/test_schedule_control_route_demand_coverage_recommendations.py`<br>`tests/unit/test_schedule_draft_workbook.py`<br>`tests/unit/test_api_route_registry.py`<br>`tests/runtime/api/test_workpages_route_demand_contract.py`<br>`tests/runtime/api/test_workpages_artifact_schedule_contract.py`<br>`frontend/src/pages/logisticsRouteDemandWorkpagePage.test.tsx`<br>`frontend/src/pages/logisticsScheduleArtifactWorkpagePage.test.tsx`<br>`tests/runtime/contracts/test_frontend_snapshot_fixtures.py` | regenerated route-demand and driver-preferences workpage snapshots with existing-week coverage handoff truth<br>successor `planning.draft_weekly_schedule.workbook` draft versions created only from backend coverage apply<br>canonical route-demand coverage context and grouped recommendation payloads visible in API/runtime tests | pre-publish weekly-draft only<br>frontend selection state is not schedule truth<br>coverage apply revalidates backend candidates and creates the successor draft server-side<br>route-demand dependency is explicitly repinned on the successor schedule draft<br>future-week Stage04 activation remains a separate path<br>no live-dispatch/runtime-status/contact-data/manual-CTA completion is implied | implemented (bounded subset) |
