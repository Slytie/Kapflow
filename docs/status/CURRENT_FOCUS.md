...
- The EPIC-141 RF pagination/hydration pair is now closed: `TASK-0372` routes workflow-run and subject artifact list API branches through SQL-level page adapters with deterministic ordering and kind filtering, and `TASK-0373` extends the shared batch relation hydrator with optional internal human-task/flag summaries while preserving the default public payload shape. `TASK-0374` remains the next open EPIC-141 RF row; no bulk ingest seam, public route, frontend route, migration, official pointer, raw corpus import, reviewed-baseline claim, or CAPEX runtime/product activation is implied.
