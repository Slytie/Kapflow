...
- The EPIC-141 W2 source-state pair is now closed through `TASK-0393`: `TASK-0374` records the internal planning-only bulk ingest adapter seam over sanitized staged descriptors, `TASK-0391` reconciles the content identity/source occurrence runtime schema row to existing `TASK-0564` physical evidence without a duplicate migration, `TASK-0392` adds internal same-project source occurrence relation state, and `TASK-0393` adds internal ingest batch/job/attempt/log state. Locator unions, queue workers, extractor/OCR/search runtime, public routes, frontend routes, official pointers, raw corpus import, reviewed-baseline claims, and CAPEX runtime/product activation remain blocked.
