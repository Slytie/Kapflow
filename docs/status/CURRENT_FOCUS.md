...
- EPIC-140 is now closed as the CAPEX project/access tranche. `TASK-0261` through `TASK-0265`, `TASK-0371`, `TASK-0381` through `TASK-0390`, and `TASK-0563` are closed with durable `capex_projects`, direct `project_memberships`, minimal project APIs, project-bound workflow-run creation, project-scoped child routes, the first max-five project selector/dashboard, a shared project-scope helper, project-scoped official pointer families, a neutral domain-runtime manifest skeleton, a ready-state logistics manifest inventory, an incubation-state CAPEX manifest inventory, approval-effect registry shadow parity, project authorization CED, projection-backed `AuthorizedProjectsQuery`, storage/blob custody CED, pilot storage gate checklist, W1 code pattern register, W1 closeout review, auth-before-read project visibility checks, and rebuildable physical authorization projection runtime state. CAPEX activation remains blocked by later source-governance, workflow/workpage, storage evidence or waiver, release/capacity, and production-preflight gates.
