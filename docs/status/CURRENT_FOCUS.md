...
- EPIC-140 is now the active CAPEX project/access tranche. `TASK-0261` through `TASK-0265`, `TASK-0371`, and `TASK-0381` through `TASK-0388` are closed with durable `capex_projects`, direct `project_memberships`, minimal project APIs, project-bound workflow-run creation, project-scoped child routes, the first max-five project selector/dashboard, a shared project-scope helper, project-scoped official pointer families, a neutral domain-runtime manifest skeleton, a ready-state logistics manifest inventory, an incubation-state CAPEX manifest inventory, approval-effect registry shadow parity, project authorization CED, `AuthorizedProjectsQuery` prototype, storage/blob custody CED, pilot storage gate checklist, and auth-before-read project visibility checks. Later EPIC-140 work still owns physical authorization projection runtime state, real pilot storage evidence or waiver, richer CAPEX workpages/projections, raw-corpus governance dependencies, and CAPEX activation.
