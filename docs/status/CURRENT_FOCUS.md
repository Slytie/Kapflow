...
- The EPIC-141/143 chunk-index and owner-interface pair is now closed: `TASK-0272` records deterministic `capex.document_chunk_index.v1`, `capex.document_search_index.v1`, and `capex.evidence_binding_index.v1` planning outputs from sanitized text/page manifest basis rows, and `TASK-0288` records the planning-only Owner Interface Resolution workflow contract/helper with `distributed_requirement_register`, `interface_register`, and `owner_interface_flags` outputs. This opened `TASK-0273` batch artifact link/provenance hydration as the next EPIC-141 ingest task; at that closeout point, `TASK-0289` was still blocked by `TASK-0285` lifecycle navigation state. No search runtime, vector store, retrieval runtime, evidence-review runtime, public routes, authored workflow activation, canonical responsibility assignment, physical closure snapshots, stale/reopen policy, reviewed evidence sufficiency, approval mutation, official pointer creation, or CAPEX runtime/product behavior is activated.
- The EPIC-141/143 batch-hydration and lifecycle-navigation pair is now closed: `TASK-0273` adds bounded project artifact page and batch link/provenance hydration helpers with query-count and 5k synthetic artifact evidence, and `TASK-0285` records the planning-only Lifecycle Stage State workflow contract/helper with `lifecycle_stage_state`, `stage_readiness_matrix`, and `lifecycle_navigation_flags` outputs. No raw corpus import, parser/OCR runtime, async processing runtime, new public routes, frontend routes, migrations, event-registry change, authored workflow activation, waterfall gate truth, reviewed baseline creation, official pointer creation, or CAPEX runtime/product behavior is activated.
- The EPIC-141/143 async-job and project-snapshot pair is now closed: `TASK-0274` records planning-only async document-processing job runtime outputs with deterministic idempotency, command receipts, retry/resume/cancel/progress, and no-duplicate planned task/artifact refs; `TASK-0289` records the planning-only Project State Snapshot workflow contract/helper with `project_state_snapshot`, `project_closure_vector`, and `project_state_snapshot_flags` outputs. This leaves physical ingest job tables/workers, parser/OCR runtime, authored Snapshot workflow activation, physical closure snapshots, official project-state truth, reviewed baseline creation, official pointer creation, and CAPEX runtime/product behavior blocked.
- The EPIC-141 W2 source-state pair is now closed through dependency-ready `TASK-0405`: `TASK-0374` records the internal planning-only bulk ingest adapter seam over sanitized staged descriptors, `TASK-0391` reconciles the content identity/source occurrence runtime schema row to existing `TASK-0564` physical evidence without a duplicate migration, `TASK-0392` adds internal same-project source occurrence relation state, `TASK-0393` adds internal ingest batch/job/attempt/log state, `TASK-0394` records metadata-only archive lineage outputs over existing relation rows, `TASK-0395` records manifest-generation attestation outputs from physical corpus rows only, `TASK-0396` hardens command receipts with canonical `sha256:` input hash/profile validation and corruption rejection, `TASK-0397` adds durable EffectLedger entries plus a guarded mutation helper, `TASK-0398` adds internal ToolExecutionAttempt lease-token stale-completion rejection while preserving legacy logical completion compatibility, `TASK-0399` adds internal project-scoped ingest/pointer runtime slot primitives, `TASK-0400` adds the internal RuntimeOutbox relay scaffold over canonical `timeline_events` and `consumer_cursors`, `TASK-0401` adds immutable ArtifactVersion identity profile/digest metadata without officialness fields, `TASK-0403` adds append-only pointer event and pointer-family policy state, and `TASK-0405` adds the submitted/runtime generated artifact schema boundary. `TASK-0402` remains open until `TASK-0406` closes. Locator unions, queue workers, extractor/OCR/search runtime, archive extraction runtime, external relay/bus wiring, broad worker migration, broad command-enforcement wiring, pointer-promotion service work, validation-run vector work, runtime-computed provenance hardening, public routes, frontend routes, official pointers, raw corpus import, reviewed-baseline claims, and CAPEX runtime/product activation remain blocked.
- The EPIC-151 RiskSignal / W8 CEO freshness pair is now closed: `TASK-0539` records the planning-only RiskSignal contract/helper for deterministic `capex.risk_signal_register.v1` rows over Risk/CEO workflow outputs; `TASK-0540` records the planning-only W8 CEO transparency freshness companion contract/schema/helper over the existing `capex.ceo_transparency_snapshot.v1`, Risk/CEO workflow outputs, and optional RiskSignal outputs. CEO cockpit UI, public routes, frontend routes, runtime risk engine, authored workflow activation, numeric threshold signoff, procurement workflow activation, ERP/accounting behavior, official pointers, closure snapshots, external-system activation, and CAPEX runtime/product behavior remain blocked.
