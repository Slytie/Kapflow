...
- The EPIC-141 bulk-ingest/W2 schema pair is now closed: `TASK-0374` records the internal planning-only bulk ingest adapter seam over sanitized staged descriptors, and `TASK-0391` reconciles the content identity/source occurrence runtime schema row to existing `TASK-0564` physical evidence without a duplicate migration. Source occurrence relations, ingest job tables, locator unions, extractor runtime, public routes, frontend routes, official pointers, raw corpus import, reviewed-baseline claims, and CAPEX runtime/product activation remain blocked.
