...
- The EPIC-141/143 text/page and assumption-closure pair is now closed: `TASK-0271` records deterministic `capex.document_text_extract.v1` and `capex.document_page_manifest.v1` planning outputs from sanitized document-manifest basis rows, and `TASK-0287` records the planning-only Assumption Closure workflow contract/helper with `counterparty_assumption_register`, `assumption_closure_matrix`, and `assumption_flags` outputs. This opens `TASK-0272` chunk/search/evidence-binding index work and unblocks `TASK-0288` Owner Interface Resolution; `TASK-0285` remains TODO as lower-priority lifecycle navigation state. No parser/OCR runtime, async extraction jobs, public routes, authored workflow activation, physical closure snapshots, stale/reopen policy, reviewed evidence sufficiency, approval mutation, official pointer creation, or CAPEX runtime/product behavior is activated.
