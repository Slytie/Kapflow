...
- EPIC-139 is reopened as RED/active after the `2026-06-04` redo package classified this checkout as State B: hook/registry seams existed, but platform defaults still loaded logistics packs. Old EPIC-139 task closeouts remain historical evidence; do not rewrite them. `TASK-0643` is the active neutral-default repair: approval and workpage defaults must be platform-neutral, logistics behavior must be explicitly activated, and tests/audit must reject the old false-green default-import pattern.
- `TASK-0643` keeps EPIC-139 in active review until architecture/QA accepts the reopened gate. EPIC-150 wording correction and final Python/Node supported-environment acceptance remain follow-up work outside this P0 repair.
