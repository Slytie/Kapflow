...
- EPIC-139 is now the active CAPEX domain-boundary cleanup tranche; `TASK-0245` is closed with file-backed, evented, provenance-linked, receipt-scoped weekly Stage04 generated outputs, `TASK-0246` is closed with handoff effect scopes carrying source truth, target partition, and policy version metadata, `TASK-0247` is closed with seven file-backed weekly seed artifacts created once per planning-week materialization, `TASK-0248` is closed with stale policy-state handling for live-dispatch republish-after-prepare cases, `TASK-0249` is closed with file-backed notify-only manifests plus a shared-env late-report conflict guard, `TASK-0250` is closed with explicit logistics calendar, republish, and late-report policy objects including ISO rollover coverage, `TASK-0251` is closed with canonical weekly-to-weekly route-demand carry-forward that prepares target input truth without Stage04 auto-run or approval side effects, `TASK-0252` is closed with a read-only logistics reconciler dry-run report for missing seeds/runs, late reports, and stale/drifted handoff rows, `TASK-0253` is closed with `/` as the shared-env-safe operator home plus hidden actor switching and failure-state findings, `TASK-0257` is closed with generic approval response isolated from logistics side effects through an explicit domain hook registry and ADR-005, `TASK-0258` is closed with logistics workpage/action rules registered behind a domain-neutral registry seam, `TASK-0259` is closed with logistics docs classified under `docs/domains/logistics/`, `TASK-0260` is closed with explicit platform-substrate and logistics-regression test lanes, `TASK-0369` is reconciled to DONE against the already-landed approval-response hook extraction evidence, `TASK-0370` is closed with logistics workpage descriptors registered behind a domain-neutral descriptor registry, and `TASK-0561` is reconciled to DONE as the remaining imported approval-response duplicate row.
- No open EPIC-139 imported domain-boundary tasks remain.
- EPIC-140 is now the active CAPEX project/access tranche. `TASK-0261` and `TASK-0262` are closed with durable `capex_projects`, direct `project_memberships`, minimal project APIs, project-bound workflow-run creation, and auth-before-read project visibility checks. Later EPIC-140 work still owns project-scoped child APIs, authorization projections, max-five project selector UX, official pointer families, and CAPEX activation.
