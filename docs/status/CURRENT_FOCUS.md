...
- EPIC-140 project/access foundation and SME-RP scope hierarchy contract are closed, but the SME-RP RACI addendum remains open. `TASK-0261` through `TASK-0265`, `TASK-0371`, `TASK-0381` through `TASK-0390`, `TASK-0563`, and `TASK-0649` are closed with durable `capex_projects`, direct `project_memberships`, minimal project APIs, project-bound workflow-run creation, project-scoped child routes, the first max-five project selector/dashboard, a shared project-scope helper, project-scoped official pointer families, a neutral domain-runtime manifest skeleton, ready/incubation domain manifests, approval-effect registry shadow parity, project authorization CED, projection-backed `AuthorizedProjectsQuery`, storage/blob custody CED, pilot storage gate checklist, W1 evidence, physical authorization projection runtime state, and planning-only `capex_scope` hierarchy contract. `TASK-0650` remains open for RACI/role-permission mapping; CAPEX activation remains blocked by later source-governance, workflow/workpage, storage evidence or waiver, release/capacity, and production-preflight gates.
