...
## 2026-06-23 (EPIC-141 W2 pointer history and generated artifact boundary closeout)
- Task-order decision: `TASK-0403` and `TASK-0405` close as the next dependency-ready EPIC-141 W2 pair. `TASK-0402` remains open because its source dependency includes `W2-S16` / `TASK-0406`, which is still open.
- Pointer event decision: `TASK-0403` closes `ARCH-W2-S13` by adding `artifact_pointer_events`, `artifact_pointer_family_policies`, runtime schemas, Alembic/bootstrap/model parity, `docs/planning/capex_source_ingest/ARTIFACT_POINTER_EVENT_POLICY_CONTRACT.yaml`, and `onetruth.infrastructure.repositories.artifact_pointer_events`.
- Pointer-boundary decision: `artifact_pointers` remains the current pointer row. The new pointer-event helper records append-only history and pointer-family policy rows only; it replays exact duplicates, rejects conflicts, validates tenant/domain/project scope, generation fields, and `sha256:` digests, bans raw material, and does not mutate current pointers or emit timeline events.
- Generated-artifact boundary decision: `TASK-0405` closes `ARCH-W2-S15` by adding `schemas/runtime/capex_submitted_generated_artifact.schema.json`, `schemas/runtime/capex_runtime_generated_artifact_view.schema.json`, `docs/planning/capex_generated_artifacts/SUBMITTED_RUNTIME_GENERATED_ARTIFACT_CONTRACT.yaml`, and `onetruth.capex_platform.generated_artifact_submission`.
- Submission-boundary decision: submitted generated artifacts may include only artifact kind/role, SourceRefs, input digests, validation summary, and payload. Runtime-owned fields such as artifact version IDs, storage refs, content/identity digests, pointer fields, officialness/status fields, event IDs, and timestamps are rejected; runtime views expose persisted artifact metadata without promotability, evidence-sufficiency, reviewed-baseline, pointer, or officialness authority.
- Activation decision: this closeout adds internal schemas, helpers, contracts, tests, docs, and schema parity only. It adds no public route, frontend route, event-registry change, pointer-promotion service activation, broad generated-artifact migration, raw corpus import, official pointer, reviewed-baseline claim, production approval, or CAPEX runtime/product activation.
