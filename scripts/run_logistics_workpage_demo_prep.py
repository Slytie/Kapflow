...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="ServiceDateID for the current reporting run.",
    )
    parser.add_argument(
        "--output-json",
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip creating the driver-preferences snapshot.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        prepared = prepare_logistics_workpage_demo(
            ...
            include_driver_preferences=not bool(args.no_driver_preferences),
        )
    finally:
        connection.close()
    ...
    result = {
        "status": "ok",
        "command": "logistics-workpage-demo.prepare",
        **prepared,
    }
    if args.output_json:
