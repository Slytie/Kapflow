# P28 — Integration / ERP Sync Status
...
| Primary role | Finance, admin, PM |
| Primary user question | What is official in CAPEX versus posted in downstream systems? |
| Page archetype | sync state and exception table |
| MVP band | Near-MVP/Post-MVP |
| Primary state objects | handoff_manifests, sync_events, external_ids |
| Evidence pattern | Last sync, queued/failed/retried, external IDs visible |
| Mobile behavior | Read-only sync exceptions |
...
- Page-specific sections: Handoff manifests; Sync event table; External IDs; Failed/queued/retried exceptions; Reconciliation drawer.
...
- Sync event table
- External IDs
- Failed/queued/retried exceptions
...
|---|---|---|
| `object` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `capex_state` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `external_system` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `external_id` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `last_sync` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
| `sync_state` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
| `failure_reason` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `retry_count` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
...
|---|---|---|
| Sync detail | Shows sync detail for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| CAPEX basis | Shows capex basis for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| External receipt | Shows external receipt for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Failure analysis | Shows failure analysis for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Retry/reconcile actions | Shows retry/reconcile actions for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
| `open_exception` | Sync exception accessible. | Opens exception drawer. |
| `reconcile` | Reconciliation role; external receipt/basis selected. | Reconciliation event/task. |
| `export_handoff_manifest` | Manifest scope selected. | Export event. |
...
- official_equals_synced
- retry_without_current_manifest
- hide_failed_sync
...
- Mobile: Read-only sync exceptions
...
|---|---|---|---|
| P28-T01 | basis_visibility | Open page with seeded data. | All primary items show basis and freshness; items with unresolved refs are visually marked. |
| P28-T02 | drawer_evidence | Select a representative row. | Drawer opens without losing table context and includes evidence, policy checks, command panel, audit events. |
| P28-T03 | blocked_shortcut | Attempt shortcut via UI/API test harness. | UI explains blocker; backend command receipt rejects state mutation. |
| P28-T04 | stale_behavior | Inject stale trigger for page basis. | Page shows stale reason banner, disables/gates commands, and offers re-review task path. |
| P28-T05 | mobile_behavior | Open narrow viewport. | Mobile layout shows task/read-only drilldown according to contract; no required desktop-only command is hidden without route. |


## 14. Design notes

Integration latency must be explicit.
