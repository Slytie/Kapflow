...
| Primary role | PMO, engineering |
| Primary user question | What should future projects learn from this project? |
| Page archetype | lesson candidate register |
| MVP band | Post-MVP |
| Primary state objects | lesson_candidates, archetypes, checklists |
| Evidence pattern | Lesson source and project outcome basis |
| Mobile behavior | Read-only cards |
...
- Page-specific sections: Lesson candidates; Source project outcome; Archetype mapping; Future checklist impact; Promotion/rejection queue.
...
- Archetype mapping
- Future checklist impact
- Promotion/rejection queue

## 5. Primary row/card model

| Column / field | Purpose | Evidence/state behavior |
|---|---|---|
| `lesson_candidate` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `source_project` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
| `archetype` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
...
| `reuse_scope` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
...
|---|---|---|
| Lesson detail | Shows lesson detail for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
| Applicable archetypes | Shows applicable archetypes for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Checklist/template changes | Shows checklist/template changes for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Promotion policy | Shows promotion policy for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
|---|---|---|
| `promote_lesson` | Lesson reviewed; evidence and applicability defined. | Lesson library entry. |
| `reject_lesson` | Reason provided. | Lesson candidate rejected. |
| `link_fixture` | Fixture/test reference available. | Fixture link event. |
| `create_future_checklist_item` | Lesson has actionability and scope. | Checklist item draft. |
...
- auto_promote_ai_lesson
- generalize_without_evidence
...
|---|---|---|---|
| P26-T01 | basis_visibility | Open page with seeded data. | All primary items show basis and freshness; items with unresolved refs are visually marked. |
| P26-T02 | drawer_evidence | Select a representative row. | Drawer opens without losing table context and includes evidence, policy checks, command panel, audit events. |
| P26-T03 | blocked_shortcut | Attempt shortcut via UI/API test harness. | UI explains blocker; backend command receipt rejects state mutation. |
| P26-T04 | stale_behavior | Inject stale trigger for page basis. | Page shows stale reason banner, disables/gates commands, and offers re-review task path. |
| P26-T05 | mobile_behavior | Open narrow viewport. | Mobile layout shows task/read-only drilldown according to contract; no required desktop-only command is hidden without route. |


## 14. Design notes

Capture early, build library later.
