...
| Primary role | PM, executive |
| Primary user question | Where are we in the CAPEX lifecycle without forcing waterfall semantics? |
| Page archetype | stage map + blocker explanation |
| MVP band | MVP |
| Primary state objects | lifecycle_stage_projection, stage_readiness, blockers |
| Evidence pattern | Stage chips show reviewed/official/stale/blocked basis |
| Mobile behavior | Read-only overview |
...
- Page-specific sections: Lifecycle strip; Stage readiness table; Blocker explanation; Stage evidence coverage; Reopen triggers.
...
|---|---|---|
| `stage` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `state` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `reviewed_baseline` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `official_baseline` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `blockers` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `stale_triggers` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `open_tasks` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `evidence_coverage` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
...
|---|---|---|
| Stage definition | Shows stage definition for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Evidence supporting stage state | Shows evidence supporting stage state for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Blockers | Shows blockers for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Reopen logic | Shows reopen logic for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Next actions | Shows next actions for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
|---|---|---|
| `open_stage_workpage` | Stage projection exists. | Navigation only. |
| `explain_blocker` | Blocker selected. | No mutation; opens blocker drawer. |
| `request_re_review` | Stale/uncertain item selected. | Re-review task created. |
| `create_stage_task` | Stage blocker selected. | Stage task created. |
...
- manual_done_toggle
- force_waterfall_lock
- close_stage_without_evidence
...
|---|---|---|---|
| P10-T01 | basis_visibility | Open page with seeded data. | All primary items show basis and freshness; items with unresolved refs are visually marked. |
| P10-T02 | drawer_evidence | Select a representative row. | Drawer opens without losing table context and includes evidence, policy checks, command panel, audit events. |
| P10-T03 | blocked_shortcut | Attempt shortcut via UI/API test harness. | UI explains blocker; backend command receipt rejects state mutation. |
| P10-T04 | stale_behavior | Inject stale trigger for page basis. | Page shows stale reason banner, disables/gates commands, and offers re-review task path. |
| P10-T05 | mobile_behavior | Open narrow viewport. | Mobile layout shows task/read-only drilldown according to contract; no required desktop-only command is hidden without route. |


## 14. Design notes

Lifecycle is context, not truth engine.
