...
| Primary role | PM, steering group |
| Primary user question | Which option should become the selected planning baseline and why? |
| Page archetype | weighted matrix + recommendation flow |
| MVP band | Near-MVP |
| Primary state objects | decision_criteria, scores, weights, option_risks, recommendation |
| Evidence pattern | Matrix cells link to evidence and assumptions |
| Mobile behavior | Review-only decision task |
...
- Page-specific sections: Criteria and weights; Option scoring matrix; Evidence per score; Risk/pros/cons comparison; Recommendation; Approval/promotion flow.
...
|---|---|---|
| `criterion` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `weight` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `option_a_score` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `option_b_score` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `option_c_score` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `evidence_basis` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
| `risk_note` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `review_state` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `decision_impact` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
...
|---|---|---|
| Criterion definition | Shows criterion definition for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Score evidence | Shows score evidence for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Weight rationale | Shows weight rationale for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Sensitivity to weight changes | Shows sensitivity to weight changes for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Recommendation audit | Shows recommendation audit for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
|---|---|---|
| `score_option` | Matrix draft current; criterion editable. | Draft score saved with evidence/comment. |
| `request_review` | Draft complete enough for review. | Review task created. |
| `approve_selection` | Approver route current; recommendation version selected. | Selection approval event. |
| `promote_selected_concept` | Selection approved; policy checks pass; stale token current. | Official selected-concept pointer generation. |
| `freeze_matrix_version` | Matrix complete enough for review. | Matrix version locked for review. |
...
- recommendation_equals_official
- hide_weights
- promote_selected_concept_without_approval
...
- Mobile: Review-only decision task
...
|---|---|---|---|
| P13-T01 | basis_visibility | Open page with seeded data. | All primary items show basis and freshness; items with unresolved refs are visually marked. |
| P13-T02 | drawer_evidence | Select a representative row. | Drawer opens without losing table context and includes evidence, policy checks, command panel, audit events. |
| P13-T03 | blocked_shortcut | Attempt shortcut via UI/API test harness. | UI explains blocker; backend command receipt rejects state mutation. |
| P13-T04 | stale_behavior | Inject stale trigger for page basis. | Page shows stale reason banner, disables/gates commands, and offers re-review task path. |
| P13-T05 | mobile_behavior | Open narrow viewport. | Mobile layout shows task/read-only drilldown according to contract; no required desktop-only command is hidden without route. |


## 14. Design notes

Recommendation, approval, and official baseline are separate.
