...
| Primary role | PM, finance, technical reviewer |
| Primary user question | Is there a technically plausible and justified reason to continue? |
| Page archetype | staged analysis workpage |
| MVP band | MVP/Near-MVP |
| Primary state objects | feasibility_finding, rough_cost_band, rough_risk_profile, business_case |
| Evidence pattern | Evidence gaps and justification type visible |
| Mobile behavior | Compact decision proposal review |
...
- Page-specific sections: Business need and decision frame; Technical feasibility findings; Rough cost band; Rough risk profile; Justification type; Decision proposal; Evidence gaps.

## 4. Primary sections

- Business need and decision frame
...
|---|---|---|
| `finding` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `type` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `rough_value_or_state` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `basis` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
| `confidence_or_forecastability` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `owner` | Supports the page question and table scanning. | Shows state label or opens drawer when decision-relevant. |
| `evidence_gap` | Supports the page question and table scanning. | Must link to evidence/basis where value affects decision. |
...
|---|---|---|
| Finding detail | Shows finding detail for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
| Assumptions | Shows assumptions for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Risk implications | Shows risk implications for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Business case impact | Shows business case impact for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
| Review/submit actions | Shows review/submit actions for the selected object. | Preserve workpage context; deep comparison can route to a full page. |
...
|---|---|---|
| `save_feasibility_draft` | Feasibility fields editable. | Feasibility draft saved. |
| `request_review` | Draft complete enough for review. | Review task created. |
| `submit_decision_proposal` | Business case fields and evidence/unknowns declared. | Decision proposal version created. |
| `request_external_quote` | Supplier/context and purpose selected. | Request/track quote task created. |
| `mark_not_forecastable` | Reason selected; evidence gap or stale basis present. | Not-forecastable state recorded. |
...
- Mobile: Compact decision proposal review
...
|---|---|---|---|
| P11-T01 | basis_visibility | Open page with seeded data. | All primary items show basis and freshness; items with unresolved refs are visually marked. |
| P11-T02 | drawer_evidence | Select a representative row. | Drawer opens without losing table context and includes evidence, policy checks, command panel, audit events. |
| P11-T03 | blocked_shortcut | Attempt shortcut via UI/API test harness. | UI explains blocker; backend command receipt rejects state mutation. |
| P11-T04 | stale_behavior | Inject stale trigger for page basis. | Page shows stale reason banner, disables/gates commands, and offers re-review task path. |
| P11-T05 | mobile_behavior | Open narrow viewport. | Mobile layout shows task/read-only drilldown according to contract; no required desktop-only command is hidden without route. |


## 14. Design notes

Interview shows feasibility is rough possibility, cost, risk, ROI/strategy.
