...
describe("logistics workpage routes", () => {
  it("navigates from the logistics demo shell to the canonical run-backed weekly review workpage", async () => {
    const user = userEvent.setup();
    setFrontendOperatorContext();
    window.history.pushState({}, "", "/demo/logistics?planning_week_id=PW-2026-W10");
    render(<App />);

    expect(await screen.findByText("Canonical run-backed workpages")).toBeInTheDocument();
  ...
  it("navigates from the logistics demo shell to the canonical run-backed end-of-day workpage", async () => {
    const user = userEvent.setup();
    setFrontendOperatorContext();
    window.history.pushState({}, "", "/demo/logistics?planning_week_id=PW-2026-W10");
    render(<App />);

    await user.click(await screen.findByRole("link", { name: "Open EOD workpage" }));
  ...
  it("keeps the demo schedule alias reachable from the logistics demo shell", async () => {
    const user = userEvent.setup();
    setFrontendOperatorContext();
    window.history.pushState({}, "", "/demo/logistics?planning_week_id=PW-2026-W10");
    render(<App />);

    expect(await screen.findByText("Compatibility alias workpages")).toBeInTheDocument();
    await user.click(await screen.findByRole("link", { name: "Open demo schedule alias" }));

    expect(await screen.findByTestId("schedule-workpage-page")).toBeInTheDocument();
    expect(window.location.pathname).toBe("/demo/logistics/workpages/schedule-v0");
  });

  it("keeps the demo EOD alias reachable from the logistics demo shell", async () => {
    const user = userEvent.setup();
    setFrontendOperatorContext();
    window.history.pushState({}, "", "/demo/logistics?planning_week_id=PW-2026-W10");
    render(<App />);

    await user.click(await screen.findByRole("link", { name: "Open demo EOD alias" }));
  ...
  it("navigates from the run-backed schedule landing to the canonical schedule artifact route", async () => {
    const user = userEvent.setup();
    setFrontendOperatorContext();
    window.history.pushState({}, "", "/runs/wr-weekly-001/workpages/schedule-v0");
    render(<App />);
